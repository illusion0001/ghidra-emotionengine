...
public class EmotionEngineAddressAnalyzer extends ConstantPropagationAnalyzer {
	...
	private static final String OPTION_NAME_ASSUME_T9_ENTRY =
		"Assume T9 set to Function entry";
	private static final String OPTION_DESCRIPTION_ASSUME_T9_ENTRY =
		"Turn on to assume that T9 is set to the entry address of a function"
		+ " when unset T9 register usage encountered";
	...
	private static final String OPTION_DESCRIPTION_RECOVER_GP =
		"Discover writes to the global GP register and assume as constant"
		+" at the start of functions if only one value has been discovered.";
	...
	public EmotionEngineAddressAnalyzer() {
		super(EmotionEngineLoader.EE_PROCESSOR_NAME);
	}

	@Override
	public boolean canAnalyze(Program program) {
		if (!EmotionEngineLoader.canLoad(program)) {
			return false;
		}
	...
	private void checkForGlobalGP(Program program, AddressSetView set, TaskMonitor monitor) {
		...
		// TODO: Use gp_value provided by MIPS .reginfo or dynamic attributes
		// - check for Elf loader symbol
	...
	public AddressSetView flowConstants(final Program program, Address flowStart,
		...
		EmotionEngineContextEvaluator eval = new EmotionEngineContextEvaluator(func, symEval);

		if (func != null) {
			flowStart = func.getEntryPoint();
			if (eval.getLocalGPValue() != null) {
				ProgramContext programContext = program.getProgramContext();
				RegisterValue gpVal = programContext.getRegisterValue(gp, flowStart);
				if (gpVal == null || !gpVal.hasValue()) {
					gpVal =
						new RegisterValue(gp,
							BigInteger.valueOf(eval.getLocalGPValue().getOffset()));
					try {
		...
		resultSet.add(eval.getCoveredSet());
	...
	private class EmotionEngineContextEvaluator extends ConstantPropagationContextEvaluator {
		private Address localGPAssumptionValue;
		private final Function func;
		private final SymbolicPropogator symEval;
		private final AddressSet coveredSet;
		private boolean mustStopNow = false; // if something discovered in processing, mustStop flag

		EmotionEngineContextEvaluator(Function func, SymbolicPropogator symEval) {
			super(trustWriteMemOption);
			this.localGPAssumptionValue = gp_assumption_value;
			this.func = func;
			this.symEval = symEval;
			this.coveredSet = new AddressSet();
		}
		...
		AddressSet getCoveredSet() {
			return coveredSet;
		}

		Address getLocalGPValue() {
			return localGPAssumptionValue;
		}
	}
